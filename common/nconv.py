...
class NConvUNetNew(nn.Module):
    def __init__(self, in_ch, out_ch, num_channels=2, pos_fn='SoftPlus'): # num_channels changed from 2 to 10
        super().__init__()
        self.__name__ = 'NConvUNetNew'

        self.nconv1 = NConv2d(in_ch, in_ch * num_channels, (5, 5), pos_fn, 'k', padding=(2,2)) # richard: weird error, used to be 2
        ...
        self.nconv7 = NConv2d(in_ch * num_channels, in_ch * num_channels, (3, 3), pos_fn, 'k', padding=(1,1))

        self.nconv8 = NConv2d(in_ch * num_channels, out_ch, (1, 1), pos_fn, 'k')
    ...
    def forward(self, x0, c0):
        ...
        # upsample 4
        xout = F.interpolate(xout, torch.Size([x0.size()[2] * 2, x0.size()[3] * 2]), mode='nearest')
        cout = F.interpolate(cout, torch.Size([x0.size()[2] * 2, x0.size()[3] * 2]), mode='nearest')
